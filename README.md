...
- **Power control**: unlike VisionCB, NUCLEO has no power input independent of its debug probe's USB link — the ST-LINK *is* the board's power. `run_nucleo_hil.py` power-cycles the board by switching the USB hub port the ST-LINK sits behind (`infrastructure/core/usb_hub_power.py`, via `uhubctl -l 3-11.2 -p 3`) — a confirmed real VBUS cut (the upstream VIA Labs VL812 hub reports `ppps`, per-port power switching support), not just a logical disconnect. This happens at the start of every run and again, automatically, if the hardware sequence hits `RECOVERY_REQUIRED` mid-run. Requires the `uhubctl` CLI (`sudo apt-get install -y uhubctl`) plus two host-level permissions already set up on this host: a udev rule granting the `plugdev` group control access to the hub (`idVendor=2109`, `idProduct=2812`), and a second udev rule (`UDISKS_IGNORE=1`, keyed on the ST-LINK's serial number) stopping udisks2/gvfs from auto-mounting the ST-LINK's own mass-storage LUN (`NOD_F446RE`) on every reconnect — both in `/etc/udev/rules.d/`, not in this repo.
- Because the ST-LINK itself disappears and reappears on every power-cycle, its `/dev/ttyACM*` index can change even though the `/dev/serial/by-id` path stays the same; `run_nucleo_hil.py` always re-resolves the device after a power-cycle rather than reusing a previous path (see `acquire_device_via_power_cycle()`).
